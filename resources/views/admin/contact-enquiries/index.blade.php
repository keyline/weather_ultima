...
@section ('content')
    ...
    <div class="mx-auto max-w-7xl space-y-6">
        <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <p class="text-sm text-slate-500">Review, respond to and manage messages sent from the website contact form.</p>
            <a href="{{ route('admin.contact-enquiries.export', $exportQuery) }}" class="admin-btn admin-btn--ghost">
        ...
        <div class="admin-card overflow-hidden">
            @if ($enquiries->isEmpty())
                <div class="p-12 text-center text-slate-500">
                    ...
                    @if ($hasAnyEnquiries)
                        <p class="mt-4 font-semibold text-slate-700">No enquiries match your filters</p>
                        <a href="{{ route('admin.contact-enquiries.index') }}" class="mt-3 inline-block text-sm font-semibold text-[#0b376d]">Clear filters</a>
                    @else
                        <p class="mt-4 font-semibold text-slate-700">No contact enquiries yet</p>
                        <p class="mt-1 text-sm">New messages from the website contact form will appear here.</p>
                    @endif
                </div>
            @else
                <form method="POST" action="{{ route('admin.contact-enquiries.bulk-destroy') }}" id="bulk-form">
                    @csrf
                    @method ('DELETE')
                    <div id="bulk-toolbar" class="hidden items-center justify-between gap-3 border-b border-slate-100 bg-sky-50/70 px-5 py-3">
                        ...
                        <div class="flex items-center gap-2">
                            ...
                            <button type="submit" formaction="{{ route('admin.contact-enquiries.export') }}" formmethod="GET" class="admin-btn admin-btn--success admin-btn--sm">
                                ...
                                @foreach ($enquiries as $enquiry)
                                    @php
                                        $payload = [
                                            ...
                                            'deleteUrl' => route('admin.contact-enquiries.destroy', $enquiry),
                                            'readUrl' => route('admin.contact-enquiries.read', $enquiry),
                                        ];
                                    @endphp
                                    <tr class="transition hover:bg-slate-50/80" data-enquiry='@json($payload)' data-read="{{ $enquiry->is_read ? '1' : '0' }}">
                                        ...
                                        <td class="px-4 py-4 align-top font-semibold text-slate-800">{{ $enquiry->name }}</td>
                                        ...
                                        <td class="px-4 py-4 align-top text-slate-700">{{ $enquiry->subject }}</td>
