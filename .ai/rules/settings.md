...
- **SiteSetting** ("General Settings") columns: `site_name`, `header_logo_path`, `footer_logo_path`, `favicon_path`, `contact_email`, `contact_phone`, `contact_address`, `social_*` (5), `whatsapp_number`. Accessors: `display_name`, `header_logo_url`, `footer_logo_url`, `favicon_url`, `social_links`, `whatsapp_link` (digits-only `https://wa.me/…`, null when `whatsapp_number` is empty — drives the floating WhatsApp button, see `.ai/rules/frontend.md`). Uploads go to the `public` disk under `site/`; replacing/removing deletes the old file.
