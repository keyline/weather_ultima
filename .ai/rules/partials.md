...
Credentials + per-station device MAC addresses live in the single-row weather_settings table (WeatherSetting::current(), keys encrypted), managed at Settings → Weather Station (admin.settings.weather.{edit,update,test}). STATIONS const maps tab label → MAC column. Each station also has an admin-uploaded photo (`*_image_path` columns, `STATION_IMAGES` const, `imageUrlFor()` / `imageUrls()`, stored under `weather-stations/` on the `public` disk, falls back to `material/images/service1.png`) — each has its **own** upload/remove form on the Weather Station settings page (`admin.settings.weather.images.{update,destroy}`, `{station}` = lowercase label slug, separate from the main credentials form so one image's error/save never touches the rest), passed to the panel partial as `$imageUrl` via `HomeController` → `$weatherStationImages`. A station with no MAC, or is_active off / missing keys, renders the "coming soon" placeholder. Ambient reports imperial; the service converts to °C / km/h / mm / hPa.
