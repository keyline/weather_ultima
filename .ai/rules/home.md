...
paths:
  ...
  - 'app/Http/Controllers/Admin/InstagramPostController.php'
...
- **`instagram_posts`** — repeatable, **fallback only** for the "Follow Us on Instagram" carousel (see `.ai/rules/partials.md` — `InstagramFeedService` pulls the real feed live from the Instagram Graph API when configured; this table is just what shows when that isn't set up or is unreachable). Same shape as `brand_logos` (`image`, `link_url` nullable, `display_order`, `is_enabled`; `enabled()` + `ordered()` scopes; `image_url` accessor). Images stored on the `public` disk under `instagram-posts/`. Managed at **Home → Instagram Grid (fallback)** (`admin.home.instagram.*`). No seeded default — the section is wrapped in `@if ($instagramPosts->isNotEmpty())` and simply doesn't render until there's something to show from either source.

## Admin
Routes under `Route::prefix('home')->name('home.')`: `banner.{edit,update}`, `founder.{edit,update}` (single-form),
and `logo` / `core-values` / `instagram` resources (`->except('show')` + `toggle` + `bulk-destroy` declared before the resource,
params `brand_logo` / `core_value` / `instagram_post`). Banner + founder are single `<form>` pages; logo + core-values + instagram clone the
...
`HomeController@index` passes `$home`, `$brandLogos`, `$coreValues`, `$instagramPosts` (from `InstagramFeedService::homepageItems()`, not the model directly — see `.ai/rules/partials.md`) (+ `$testimonials`). The five `.dim-card`s all
carry `style="--dim-bg: url('{{ $card->image_url ?? asset('material/images/service1.png') }}')"`. Media
strip uses `@forelse` with a single static fallback. Core Values is wrapped in `@if (...->isNotEmpty())`; the Instagram section is an `owl-carousel` (`.wx-insta-carousel`, initialised in `main.js` alongside `.wx-testimonial-carousel`), also wrapped in `@if ($instagramPosts->isNotEmpty())`.
Seeders (`HomeSettingSeeder`, `BrandLogoSeeder`, `CoreValueSeeder`) carry the original content so nothing changes visually — there is no `InstagramPostSeeder`.
