...
- **`instagram_posts`** — repeatable, powers the "Follow Us on Instagram" grid. Same shape as `brand_logos` (`image`, `link_url` nullable, `display_order`, `is_enabled`; `enabled()` + `ordered()` scopes; `image_url` accessor). Images stored on the `public` disk under `instagram-posts/`. Managed at **Home → Instagram Grid** (`admin.home.instagram.*`). Unlike brand logos there is **no seeded fallback** — the section is wrapped in `@if ($instagramPosts->isNotEmpty())` and simply doesn't render until an admin uploads photos.
...
`HomeController@index` passes `$home`, `$brandLogos`, `$coreValues`, `$instagramPosts` (+ `$testimonials`). The five `.dim-card`s all
carry `style="--dim-bg: url('{{ $card->image_url ?? asset('material/images/service1.png') }}')"`. Media
strip uses `@forelse` with a single static fallback. Core Values and Instagram Grid sections are each wrapped in `@if (...->isNotEmpty())`.
